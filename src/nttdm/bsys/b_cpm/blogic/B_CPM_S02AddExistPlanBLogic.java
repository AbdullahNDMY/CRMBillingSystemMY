 ...
 * @version 1.1
 */
public class B_CPM_S02AddExistPlanBLogic extends AbstractB_CPM_S02AddPlanBLogic {
    ...
    public BLogicResult execute(HashMap<String, Object> param) {
        ...
        String stridCustPlanLinkList=param.get("idCustPlanLinks").toString();
        ...
        String[] idCustPlanLinkList=stridCustPlanLinkList.split(",");
        ...
        //get sub plan list
        List<CustomerSubPlan> subPlans = util.getExistSubPlanByIdList(idCustPlanLinkList);
        for (CustomerSubPlan subPlan : subPlans) {
            if ("0".equals(serviceMultiPln)) {
                subPlan.setItemDesc(subPlan.getItemDesc());
            }
            subPlan.setSubPlanDetails(util.getCustomerSubPlanDetail(subPlan));
            subPlan.setQuantity(subPlan.getQuantity());
            subPlan.setUnitPrice(subPlan.getUnitPrice());
            subPlan.setIsDiscountOneTime(subPlan.getIsDiscountOneTime());
            subPlan.setItemDesc(subPlan.getItemDesc());
            subPlan.setJobNo(subPlan.getJobNo());
            subPlan.setDiscamount(subPlan.getDiscamount());
            subPlan.setIsDisplayJobNo(subPlan.getIsDisplayJobNo());
            //wcbeh@20161117 Sub Location Fix for Draft - Add Existing
            subPlan.setSubLocation(subPlan.getSubLocation());
