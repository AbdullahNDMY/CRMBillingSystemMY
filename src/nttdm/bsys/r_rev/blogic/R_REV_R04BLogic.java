...
public class R_REV_R04BLogic {
	...
	protected Integer doSummaryExcel(String periodFrom, String periodTo, String reportMonth) throws Exception{
		String filePath = queryDAO.executeForObject("R_REV.GET_FILE_PATH_REV04BL", null, String.class);
		Integer fileCount = 0;
		...
		List<Map<String, Object>> consolidList = queryDAO.executeForMapList("R_REV.GET_CONSOLIDATE_REV04BL", param);
		List<Map<String, Object>> amountList = queryDAO.executeForMapList("R_REV.GET_AMOUNT_REV04BL", param);
		List<Map<String, Object>> totalList = queryDAO.executeForMapList("R_REV.GET_TOTAL_REV04BL", param);
		List<Map<String, Object>> summaryList = queryDAO.executeForMapList("R_REV.GET_SUMMARY_REV04BL", param);
		
		//--- Get Template Start ---
		InputStream is = this.getClass().getResourceAsStream("/Template-Consolidation-Final.xlsx");
		...
		for(Map<String, Object> amlist : amountList){
			...
			columnDateCell = columnDateRow.getCell(12);
			columnDateCell.setCellValue(outsdf.format(calendar.getTime()));
			}
			columnDateCell = columnDateRow.getCell(13 + cellIndex);
		...
		for(Map<String, Object> conlist : consolidList){
		     if(count==0){
				...
				XSSFCell printDateCell = categoryRow.getCell(25);
		       ...
		       //Shift row by 1
				...
				ServiceCell.setCellValue((String)conlist.get("SVC_GRP_NAME"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//ProductCode
				ServiceCell = ServiceRow.createCell(1);	
				ServiceCell.setCellValue((String)conlist.get("PRODUCT_CODE"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//service
				ServiceCell = ServiceRow.createCell(2);	
				ServiceCell.setCellValue((String)conlist.get("PRODUCT_DESC"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//Project
				ServiceCell = ServiceRow.createCell(3);		
				ServiceCell.setCellValue((String)conlist.get("JOB_NO"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//name
				ServiceCell = ServiceRow.createCell(4);		
				ServiceCell.setCellValue((String)conlist.get("CUST_NAME"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//Location
				ServiceCell = ServiceRow.createCell(5);		
				ServiceCell.setCellValue((String)conlist.get("LOCATION_S"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//Co. Category
				ServiceCell = ServiceRow.createCell(6);		
				ServiceCell.setCellValue((String)conlist.get("CO_CATEGORY_S"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//co. Sub Category
				ServiceCell = ServiceRow.createCell(7);		
				ServiceCell.setCellValue((String)conlist.get("CO_SUB_CATEGORY_S"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//co. Type
				ServiceCell = ServiceRow.createCell(8);		
				ServiceCell.setCellValue((String)conlist.get("CO_TYPE_S"));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//Service Start/End
				ServiceCell = ServiceRow.createCell(9);		
				ServiceCell.setCellValue(convertClobToString((Clob)conlist.get("SVC_START_END_S")));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//Billing Instruction
				ServiceCell = ServiceRow.createCell(10);		
				ServiceCell.setCellValue(convertClobToString((Clob)conlist.get("BILL_INSTRUCT_S")));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));
				//plan
				ServiceCell = ServiceRow.createCell(11);		
				ServiceCell.setCellValue(convertClobToString((Clob)conlist.get("PLAN_S")));
				ServiceCell.setCellStyle((XSSFCellStyle)cellStyle.get("borderCell"));   
				
				//Double sumTemp =0.00;
			for (int j = 0; j < 13; j++) {
				String colNm = "AMOUNT" + j;
				ServiceCell = ServiceRow.createCell(12 + j);
				Double temp=Double.parseDouble(conlist.get(colNm).toString());
			...
			String cellFromForSubTotal = CellReference.convertNumToColString(columnBeforeDate + 1) + (rowStartList + count + 1); 
			String cellToForSubTotal = CellReference.convertNumToColString(columnBeforeDate + dateColumn) + (rowStartList + count + 1); 
			String formula = "SUM(" + cellFromForSubTotal + ":" + cellToForSubTotal + ")";
		...
		wb.removeSheetAt(0);
		// set sheet name
		wb.setSheetName(0, "consolidation");
		
		// output to new Excel file
		String fileName = "Revenue_Report_consolidation_" + reportMonth + ".xlsx";
