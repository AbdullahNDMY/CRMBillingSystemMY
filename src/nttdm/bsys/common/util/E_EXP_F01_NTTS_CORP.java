 ...
 * #146 E-EXP-F01_NTTS_CORP
 * 
 * @author davidd
 */
public class E_EXP_F01_NTTS_CORP {
    /**
     * batch_id
     */
    ...
    public void e_exp_execute() {
        ...
        tset.setMonthyear(new SimpleDateFormat("MM/yyyy").format(esetRundate) + " - Corporate"); // add invoice period after the monthYear
        ...
        if (batch_id >= 0) {
            ...
            if (!CommonUtils.isEmpty(export_path)) {
                if (new File(export_path).exists()) {
                    ...
                    StringBuffer sBFileName = new StringBuffer(export_path).append("/PeopleSoft_Billing_CORP_") // 1.file name add : _CORP
                            .append(date_STR).append("_").append(nowFormat.format(now)).append("_").append(batch_id)
                            .append(".txt");
                    StringBuffer sBFileNameNotIncludePath = new StringBuffer().append("/PeopleSoft_Billing_CORP_") // 1.file name add : _CORP
                            .append(date_STR).append("_").append(nowFormat.format(now)).append("_").append(batch_id)
                            .append(".txt");
                    ...
                    List<E_EXP_F01_SQL1_Bean> listheader = queryDAO.executeForObjectList(
                            "SELECT.BSYS.E_EXP_F01_NTTS_CORP.SQL001", date_STR);
                    ...
                    if (listheader != null && listheader.size() > 0) {
                        try {
                            ...
                            for (int i = 0; i < listheader.size(); i++) {
                                ...
                                detail_List = queryDAO.executeForObjectList("SELECT.BSYS.E_EXP_F01_NTTS_CORP.SQL002", sql2_input);
                                ...
                                if(taxCodeList.size()>1){
                                    ...
                                    for (int n = 0; n < taxCodeList.size(); n++) {
                                        //IS Z.TAX_CODE='T07'
                                        if(this.tax_code.equals(taxCodeList.get(n))){
                                            //One Billing Document
                                            Map<String, Object> billAmtMap=getBillAmt(isTaxCodeSR,this.tax_code,"",detail_List);
                                            ...
                                            for(int p=0;p<detail_List.size();p++){
                                                if(CommonUtils.toString(detail_List.get(p).getTAX_CODE()).equals(this.tax_code)){
                                                    //Get last detail
                                                    detail =detail_List.get(p);
                                                }
                                            }
                                            subBillDocumentHeader(header, detail, busUnit, accRev,
                                                    defCur,exportAmt,billAmount,billRef,originCode,taxCodeList.get(n),fw);
                                            //Select * from R2 where SUBSTR(R.PRODUCT_CODE,3)=Y.PRODUCT_CODE
                                            detail = new E_EXP_F01_SQL2_Bean();
                                            for(int s=0;s<detail_List.size();s++){
                                                if(CommonUtils.toString(detail_List.get(s).getTAX_CODE()).equals(this.tax_code)){
                                                    //Get detail
                                                    ...
                                                    subBillDocumentDetail(header,detail, busUnit,defCur,accRev,advAcc,projectLen,closingDate,billRef,fw);
                                                }
                                            }
                                            //Set TaxLine
                                            String itemgst=itemGST.toString();
                                            String itemexportgst=itemExportGST.toString();
                                            subBillDocumentTaxLine( header,detail,busUnit,defCur, projectLen,billRef,itemexportgst,itemgst,U_Product,Z_Project,fw);
                                        }
                                        else{
                                            //IS Z.TAX_CODE not equal 'T07'
                                            //One Billing Document
                                            isTaxCodeSR=false;
                                            Map<String, Object> billAmtMap=getBillAmt(isTaxCodeSR,taxCodeList.get(n),"",detail_List);
                                            String billRef=header.getID_REF().toString().trim().replaceAll("L", "A");
                                            if (subBillNo > 1) {
                                                billRef = billRef+"="+subBillNo;
                                            }
                                            String billAmount=billAmtMap.get("billamt").toString();
                                            String exportAmt=billAmtMap.get("expamt").toString();
                                            String originCode=billAmtMap.get("originCode").toString();
                                            ...
                                            E_EXP_F01_SQL2_Bean detail = new E_EXP_F01_SQL2_Bean();
                                            for(int p=0;p<detail_List.size();p++){
                                                if(taxCodeList.get(n).toString().equals(CommonUtils.toString(detail_List.get(p).getTAX_CODE()))){
                                                    //Get last detail
                                                    detail =detail_List.get(p);
                                                }
                                            }
                                ...
                                if(taxCodeList.size()==1){
                                    ...
                                    String Z_Project="";
                                    //Set Header
                                    int lastRecordIndex = detail_List.size() - 1;
                                    subBillDocumentHeader(header,detail_List.get(lastRecordIndex),busUnit,accRev,
                                        defCur,exportAmt,billAmount,billRef,originCode,taxCodeList.get(0),fw);
                                    E_EXP_F01_SQL2_Bean detail = new E_EXP_F01_SQL2_Bean(); 
                                    for(int s=0;s<detail_List.size();s++){
                                        //Get detail
                                        detail =detail_List.get(s);
    ...
    /**
     * append space line to FileWriter
     * 
     * @param fw
     * @throws IOException
     */
     ...
     */
    private void subBillDocumentDetail(E_EXP_F01_SQL1_Bean header,E_EXP_F01_SQL2_Bean detail,String busUnit,String defCur,
        ...
        try {
            ...
            O_Collector = "DEFAULT";
            P_PONbr = this.isNull(detail.getCUST_PO()); // IN CORP CHANGE TO R2.CUST_PO
     ...
     */
    private void subBillDocumentHeader(E_EXP_F01_SQL1_Bean header, E_EXP_F01_SQL2_Bean lastRecord, String busUnit,String accRev,String defCur,String exportAmt,String billAmount,String billRef,String originCode,String taxcode,FileWriter fw){
        try {
        String A_BusUnit = busUnit;
        String B_Origin;
        String C_CustomerId;
     ...
     */
    private void subBillDocumentTaxLine(E_EXP_F01_SQL1_Bean header,E_EXP_F01_SQL2_Bean detail,String busUnit,String defCur,int projectLen,String billRef,String itemExportGST,String itemGST,String Product,String Project,FileWriter fw){
        try {
            String A_BusUnit = busUnit;
            String B_Origin;
            String C_CustomerId;
     ...
     */
    private Map<String, Object> getBillAmt(boolean isTaxCodeSR,String taxCode,String proCode,List<E_EXP_F01_SQL2_Bean> detailsLists){
