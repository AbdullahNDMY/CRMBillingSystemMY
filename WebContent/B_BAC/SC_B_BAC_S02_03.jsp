...
<head>
	...
	<script type="text/javascript">
		...
		function checkVal(){
			var cbx = $('#deliveryEmailCBX');
			if ($('#deliveryEmailCBX').attr('checked')) {
				$('#deliveryEmailCBX').val('1');
			}
			else {
				$('#deliveryEmailCBX').val('0');
			}
		}
	</script>
</head>
...
<ts:form action="/SC_B_BAC_S02_03DSP" styleId="editForm">
	...
	<h1 class="title"><bean:message key="screen.b_bac_s01.title"/></h1>
					...
					<table cellpadding="0" cellspacing="0" width="100%">
						...
						<tr>
                            ...
                            <td valign="top">
                                ...
                                <html:hidden property="inputHeaderInfo.singPostValue"/>
                                <!-- #191 End -->
                                <!-- Email -->
                                <br/>&nbsp;&nbsp;&nbsp;&nbsp;
							    <html:checkbox value="1" name="_b_bacForm"  property="inputHeaderInfo.deliveryEmail" onclick="checkVal();">
                                </html:checkbox>
                                <html:hidden property="inputHeaderInfo.deliveryEmail" styleId="deliveryEmailCBX"/>
                            ...
                            <td>
                                ...
                                <c:if test="${not empty _b_bacForm.map.headerInfo ?(_b_bacForm.map.headerInfo.BAC_D_COUNT ne 0): (_b_bacForm.map.inputHeaderInfo.bacDCount ne 0)}">
									<c:forEach items="${LIST_TAX_TYPE}" var="item">
										<c:if test="${(not empty _b_bacForm.map.headerInfo ? _b_bacForm.map.headerInfo.TAX_TYPE : _b_bacForm.map.inputHeaderInfo.taxType) 
																		eq item.id}">
											${item.name}
										</c:if>
									</c:forEach>
									<html:hidden property="inputHeaderInfo.taxType" 
											value="${not empty _b_bacForm.map.headerInfo 
															? _b_bacForm.map.headerInfo.TAX_TYPE : _b_bacForm.map.inputHeaderInfo.taxType}"/>
								</c:if>
								<c:if test="${not empty _b_bacForm.map.headerInfo ?(_b_bacForm.map.headerInfo.BAC_D_COUNT eq 0): (_b_bacForm.map.inputHeaderInfo.bacDCount eq 0)}">
									<html:select property="inputHeaderInfo.taxType" name="_b_bacForm"
										value="${not empty _b_bacForm.map.headerInfo 
																? _b_bacForm.map.headerInfo.TAX_TYPE : _b_bacForm.map.inputHeaderInfo.taxType}" 
										styleId="cboTaxType" style="width:260px;">
		...
		<html:hidden name="_b_bacForm" property="taxTypeDisplay"/>
