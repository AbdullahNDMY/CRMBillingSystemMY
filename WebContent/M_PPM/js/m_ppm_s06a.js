...
function initload(){
	var atext=$(".serviceDetail").find(".add").val(TYPE_SUBPLAN);
	atext.html(ADD_TEXT);
	$('#serviceClone').find('.type').val(TYPE_SUBPLAN);
	var inUsedHidden = $("#serviceClone").find('.inUsedHidden').val();
	var subservice=$("#serviceClone").find(".cboSvcLevel1").val();
	if (subservice== "") {
		$('.serviceDetail').find('.add').html(ADD_TEXT);
	} else {
		$('.serviceDetail').find('.add').html(ADD_LINK);
	}
...
function changeSvcLevel1(obj) {
	...
	if(idService =="" ||undefined || null){
		$("#serviceClone").find(".add").html(ADD_TEXT);
	}
	else{
		$("#serviceClone").find(".add").html(ADD_LINK);
	}
}
